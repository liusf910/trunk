...
<script type="text/javascript">
 	  function getStorageCode(){
      	$('#storageCodeAllSignFor').val($('#storageCodeSelectAllSignFor').val());
      }
      
      function getShipperCode(){
      	$('#shipperCodeAllSignFor').val($('#shipperCodeSelectAllSignFor').val());
      }
</script>
<div class="bjui-pageHeader" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${request.contextPath}/monito/allSignFor/list"
          method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <label>超时：</label>&nbsp;&nbsp;
            <select name="dqOver" data-toggle="selectpicker" data-width="200"  title="" >
            	<option value=" " <#if infoWhere.dqOver=="">selected="selected"</#if>>全部</option>
            	<option value="0" <#if infoWhere.dqOver==0>selected="selected"</#if>>未超时</option>
                <option value="1" <#if infoWhere.dqOver==1>selected="selected"</#if>>超时</option>
                <option value="2" <#if infoWhere.dqOver==2>selected="selected"</#if>>超时一天</option>
                <option value="3" <#if infoWhere.dqOver==3>selected="selected"</#if>>超时二天</option>
                <option value="4" <#if infoWhere.dqOver==4>selected="selected"</#if>>超时三天及以上</option>
            </select>&nbsp;
            ...
            <@shiro.hasPermission name="/admin/monito/exportExcelSet">
            	<button  data-url="${request.contextPath}/monito/exportExcelSet?type=signFor&&belongTo=3-3"
        ...
        <div class="bjui-moreSearch">
        	<label>发货仓库：</label>&nbsp;&nbsp;
            <select id="storageCodeSelectAllSignFor" data-toggle="selectpicker" data-width="300"  title="" multiple onchange="getStorageCode();">
                <#if storages??>
            ...
            <input type="hidden" name="storageCode" id="storageCodeAllSignFor" value="${infoWhere.storageCode}">
            <label>快递公司：</label>&nbsp;&nbsp;
            <select id="shipperCodeSelectAllSignFor" data-toggle="selectpicker" data-width="300"  title="" multiple onchange="getShipperCode();">
                <#if couriers??>
            ...
            <input type="hidden" name="shipperCode" id="shipperCodeAllSignFor" value="${infoWhere.shipperCode}">
            <br><br>
                ...
                <tr data-id="${Info.infoId}">
                    ...
                    <#if  Info.planTime??>
                        <#if Info.systemTime?string("yyyy-MM-dd")==Info.planTime?string("yyyy-MM-dd") >
                            <td align="center" style="color: #ff0714">${Info.planTime?string("yyyy-MM-dd")}</td>
                        <#else >
