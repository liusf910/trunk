<script type="text/javascript">
   function changeBelongTo(){
   		$("#belongToStr").val($("#belongToSelect").val());
   		$("#pagerForm").submit();
   }
	...
	function exportExcel(){
        ...
        if(trueNum<=0){
         ...
         $("#belongTo").val($("#belongToSelect").val());
         $("#expNameList").val(expNameList);
         $("#expFieldName").val(expFieldName);
         $("#noexpNameList").val(noexpNameList);
         $("#noexpFieldName").val(noexpFieldName);
         $("#add_export_form").submit();
	}
</script>
<!--select选择Form-->
<form id="pagerForm" data-toggle="ajaxsearch" action="${request.contextPath}/setting/exportExcel/exportExcelSet" 
  method="get" data-toggle="validate" >
	<input type="hidden" name="belongToStr" id="belongToStr" value='${belongTo}'>
</form>

<!--add提交Form-->
 <form id="add_export_form" action="${request.contextPath}/setting/exportExcel/saveExportExcelSet" 
  method="post" data-toggle="validate" >
<input type="hidden" name="belongTo" id="belongTo" value='${belongTo}'>
    ...
    <div style="margin:15px auto 0; width:800px;">
        <fieldset>
            ...
            <table class="table table-condensed table-hover" id="xTable">
                ...
                <tr>
                <td></td>
                <td><label>模块选择：</label></td>
                <td>
                	<select id="belongToSelect" data-toggle="selectpicker" data-width="200"  title="" onchange="changeBelongTo()">
                		<option value="all" <#if belongTo=="all">selected</#if>>后台监测列表</option>
                		<option value="courier"  <#if belongTo=="courier">selected</#if>>快递监测列表</option>
            		</select>
                </td>
                <td></td>
                </tr>
                ...
                <tr>
                    <td><input type="checkbox" data-toggle="icheck" name="ids" value="shipmentsTime" alt="发货日期" data-label="发货日期" <#if returnMap??&&returnMap.shipmentsTime == "1">checked='checked'</#if>></td>
            ...
            <br/><br/>
                <div align="center">
                    <button type="button" class="btn-default" onclick="exportExcel()">保存</button>
